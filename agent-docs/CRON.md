...
through the **same monitor batch path** as {{CHANNEL_NAME}} mentions, so the agent
...
> cron), [`{{INTERFACE_DOC}}`]({{INTERFACE_DOC}}) (how to post the
> result back to {{CHANNEL_NAME}}).
...
customer's {{WORKSPACE_TZ_SOURCE}}. So `0 9 * * *` means 9am for the
...
The agent always posts to its single configured {{CHANNEL_NAME}} channel — there
...
3. The job is appended to the same `pending_messages` batch as {{CHANNEL_NAME}}
   mentions, with `type: "cron"`.
4. The agent answers it like any other message and posts the result
   using `{{INTERFACE_FILE}} say "..."`. The agent has a single
   configured {{CHANNEL_NAME}} channel, so no `-c` flag is needed. If the job has
   `thread_ts` set, the agent replies in-thread with `{{THREAD_FLAG}}`.
...
- Post the result with `{{SAY_CMD}} "..."`. If the
  job has `thread_ts` set, reply in-thread with `{{THREAD_FLAG}}`.
  Ninja has only one configured {{CHANNEL_NAME}} channel, so no `-c` flag is
  needed.
