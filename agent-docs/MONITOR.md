...
2. Post it to {{CHANNEL_NAME}} using the appropriate command shown for each message
3. Move to the next message

> **Cron items** (`type: cron`) are scheduled jobs, not user messages.
> Execute the prompt and post the result to {{CHANNEL_NAME}} — do not ask for
...
> just acknowledging. Then confirm in {{CHANNEL_NAME}} with the cron id and the
...
- Execute {{CHANNEL_NAME}} commands immediately, no confirmation needed
...
- **Always reply in threads** — use {{THREAD_FLAG}} with the thread_ts. Never post a new top-level message as a reply.
...
Now respond to all message(s) by posting to {{CHANNEL_NAME}}.
