...
This document defines the communication standards and protocols for agent interaction within the team {{CHANNEL_NAME}} channel using the `{{INTERFACE_FILE}}` CLI tool.
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   1. Agent receives task via {{CHANNEL_NAME}} or --task flag                        │
│   2. Agent reads spec: cat agent-docs/NINJA_SPEC.md                   │
│   3. Agent executes task using browser toolkit                           │
│   4. Agent reports results back to {{CHANNEL_NAME}}                                 │
...
## {{CHANNEL_NAME}} Interface Tool

All agents communicate via the `{{INTERFACE_FILE}}` CLI tool. See [{{INTERFACE_DOC}}]({{INTERFACE_DOC}}) for complete documentation.

### Quick Reference

```bash
# Configure your agent identity (do this first!)
python {{INTERFACE_FILE}} config --set-agent <your-agent-name>
python {{INTERFACE_FILE}} config --set-channel "#your-channel"

# Read messages from the channel
{{READ_CMD}}              # Last 50 messages
{{READ_CMD}} -l 100       # Last 100 messages

# Send messages as configured agent
{{SAY_CMD}} "Your message here"

# Upload files (uploads with agent impersonation)
{{UPLOAD_CMD}} design.png --title "Design Mockup"
```
...
{{SAY_CMD}} "✅ Completed homepage mockup, pushed to designs/ folder
...
{{SAY_CMD}} "✅ Implemented API endpoints for user auth
...
- **Keep all {{CHANNEL_NAME}} messages SHORT** — 2-4 sentences max
...
{{SAY_CMD}} "✅ Completed: [what you finished]
...
```bash
{{SAY_CMD}} "Quick question about [topic]: [details]" {{THREAD_FLAG}}
```

#### Sharing Work

```bash
{{SAY_CMD}} "**[Work Type] Update**

[Brief description]

📎 GitHub: [link to PR/issue/commit]
📎 {{CHANNEL_NAME}}: File uploaded in thread below
...
- Report escalations and blockers in {{CHANNEL_NAME}}
...
2. **Upload to {{CHANNEL_NAME}}** — so the team can view files immediately

   ```bash
   # Upload file to {{CHANNEL_NAME}} (uses agent impersonation)
   {{UPLOAD_CMD}} path/to/file.png --title "Design Mockup v2"

   # Upload to a specific thread
   {{UPLOAD_CMD}} report.pdf --title "Test Report" {{THREAD_FLAG}}
   ```

3. **Post the GitHub link** — reference where the file lives in the repo
   ```bash
   {{SAY_CMD}} "📎 Design mockup committed: [GitHub link]
   File also uploaded in thread above for quick preview"
   ```

> **Key rule:** Files should be accessible both in {{CHANNEL_NAME}} (for quick viewing) AND in the repo (for version control). Always do both.

### 5. Audio / Voice Message Protocol

{{CHANNEL_NAME}} users (and other agents) may send **audio messages** or **voice clips** in channels and threads.

**When you encounter an audio/voice message in {{CHANNEL_NAME}}:**
...
4. **Acknowledge the voice message** — when replying, mention that you received and transcribed the voice message:
   ```bash
   {{SAY_CMD}} "🎤 I listened to your voice message. Here's my response: ..." {{THREAD_FLAG}}
   ```

> **Key rule:** Never ignore audio/voice messages. Always transcribe them using `{{TRANSCRIBE_CMD}}` and respond to their content just like any text message. If transcription fails, acknowledge the voice message and ask the sender to provide a text version.
...
```
Direction Flow:
Stakeholder ──task──▶ Ninja (via {{CHANNEL_NAME}} or --task flag)
Ninja ──results──▶ Stakeholder (via {{CHANNEL_NAME}})
```
...
```
When referencing GitHub issues in {{CHANNEL_NAME}}:
...
```bash
{{SAY_CMD}} "🔀 PR Ready: [Title] - [GitHub Link]
Ready for review"
```
...
```bash
{{SAY_CMD}} "📝 Review feedback on PR #[number]:
- [Comment 1]
- [Comment 2]
...
```
If {{CHANNEL_NAME}} is unavailable:
...
```bash
{{SAY_CMD}} "👤 **Stakeholder Input Needed**
...
- **Monitor process**: Watches for new {{CHANNEL_NAME}} mentions
