...
- **Monitor** (`processes/monitor.py`): answers quick {{CHANNEL_NAME}} messages inline. For
...
- **Orchestrator** (`processes/orchestrator.py`), two phases per run, then exits:
  - **Phase 1 — WORK** (only if open issues): work **exactly one** issue — the
    single highest-priority open issue: understand it (read the issue + {{CHANNEL_NAME}}
