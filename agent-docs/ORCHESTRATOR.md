...
- `{{READ_CMD}} -l 50` - Read recent messages
- `{{SAY_CMD}} "message"` - Post updates
- `{{UPLOAD_CMD}} <file> --title "..."` - Upload file/screenshot
- `python {{INTERFACE_FILE}} config` - Check configuration
...
**Communicate via {{CHANNEL_NAME}} only** using `python {{INTERFACE_FILE}}`.

## {{CHANNEL_NAME}} Communication Protocol
...
2. Read {{CHANNEL_NAME}} for new requests or context
3. Do your work (browser tasks, research, screenshots, data extraction)
4. Post results to {{CHANNEL_NAME}} (short messages, attach screenshots/files)
...
- `tools/health_check.py` — System health (browser, {{CHANNEL_NAME}}, GitHub, settings)
